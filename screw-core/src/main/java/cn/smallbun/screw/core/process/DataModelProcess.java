...
public class DataModelProcess extends AbstractProcess {
    ...
    public DataModel process(DatabaseQuery query) {
        ...
        List<? extends Index> indexs = query.getIndexs();
        ...
        Map<String, ? extends List<? extends Index>> indexCaching = indexs.stream().collect(Collectors.groupingBy(Index::getTableName));
        ...
        for (Table table : tables) {
            ...
            for (Index index : indexCaching.get(table.getTableName())) {
                IndexModel indexModel = new IndexModel();
                indexModel.setIndexName(index.getIndexName());
                indexModel.setTableName(index.getTableName());
                indexModel.setColumnName(index.getColumnName());
                indexModels.add(indexModel);
            }

            // 放入索引
            tableModel.setIndexs(indexModels);
