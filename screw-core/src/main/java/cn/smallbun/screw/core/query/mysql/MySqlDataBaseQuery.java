...
public class MySqlDataBaseQuery extends AbstractDatabaseQuery {
    ...
    @Override
    public List<? extends Index> getIndexs() throws QueryException {

        ResultSet resultSet = null;
        try {
            String sql = "SELECT TABLE_NAME, INDEX_NAME, COLUMN_NAME FROM INFORMATION_SCHEMA.STATISTICS where TABLE_SCHEMA = '%s' ";
            // 拼接参数
            resultSet = prepareStatement(String.format(sql, getDataBase().getDatabase()))
                    .executeQuery();
            return Mapping.convertList(resultSet, MySqlIndexModel.class);
        } catch (SQLException e) {
            throw new QueryException(e);
        } finally {
            JdbcUtils.close(resultSet);
